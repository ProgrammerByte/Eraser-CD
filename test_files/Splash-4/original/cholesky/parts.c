...
void Partition(SMatrix M, long parts, long *T, long *assigned_ops, long *domain,
  ...
  for (;;) {
    ...
    if (maxm == 0 || (100.0 * (maxm - minm) / (double)maxm < tolerance) ||
...
long Divide(struct Chunk *root) {
  ...
  while (firstchild[first_in_super] + 1 == firstchild[first_in_super + 1]) {
    first_in_super--;
  }

  first = root->first;
  for (i = firstchild[first_in_super]; i < firstchild[first_in_super + 1];
       i++) {
    t2 = NewChunk();
    t2->last = child[i] + 1;
    t2->first = first;
