...
void multig(long my_id) {
  ...
  while ((!flag1) && (!flag2)) {
    ...
    */
    double expected = atomic_load(&(multi->err_multi));
    do {
      if (local_err <= expected)
        break;
    } while (!({
      ;
      _Bool ___b = atomic_compare_exchange_weak(&(multi->err_multi),
                                                &(expected), local_err);
      ;
      ___b;
    }));
    ...
    /* and reset global_err before we read it and decide what to do   */
  ...
  for (ic = istart; ic <= iend; ic++) {
    ...
    for (jc = jstart; jc <= jend; jc++) {
      ...
      if ((if17 == 2) || (jf == 2)) {
        ...
        s = multi->q_multi[kf][if17][jf - 1] +
            multi->q_multi[kf][if17][jf - 3] +
            multi->q_multi[kf][if17 - 1][jf - 2] +
            multi->q_multi[kf][if17 + 1][jf - 2];
      ...
      if ((if17 == 2) || (jf == 2)) {
        ...
        s = multi->q_multi[kf][if17 - 2][jf + 1] +
            multi->q_multi[kf][if17 - 2][jf - 1] +
      ...
      if ((if17 == 2) || (jf == 2)) {
        ...
        s = multi->q_multi[kf][if17 - 2][jf - 1] +
            multi->q_multi[kf][if17 - 2][jf - 3] +
            multi->q_multi[kf][if17 - 3][jf - 2] +
            multi->q_multi[kf][if17 - 1][jf - 2];
      ...
      multi->rhs_multi[krc][ic][jc] =
          i_int_factor * int1 + (1.0 - i_int_factor) * int2;
    }
  }
}
...
void intadd(long kc, long my_num) {
  ...
  for (ic = istart; ic <= iend; ic++) {
    ...
    for (jc = jstart; jc <= jend; jc++) {
      ...
      multi->q_multi[kf][if17 - 1][jf - 1] +=
          i_int_factor1 * int2 + (1.0 - i_int_factor1) * int1;
      int2 = j_int_factor1 * multi->q_multi[kc][ic + 1][jc - 1] +
             (1.0 - j_int_factor1) * multi->q_multi[kc][ic + 1][jc];
      multi->q_multi[kf][if17][jf - 1] +=
          i_int_factor2 * int2 + (1.0 - i_int_factor2) * int1;
      ...
      multi->q_multi[kf][if17 - 1][jf] +=
          i_int_factor1 * int2 + (1.0 - i_int_factor1) * int1;
      int2 = j_int_factor2 * multi->q_multi[kc][ic + 1][jc + 1] +
             (1.0 - j_int_factor2) * multi->q_multi[kc][ic + 1][jc];
      multi->q_multi[kf][if17][jf] +=
          i_int_factor2 * int2 + (1.0 - i_int_factor2) * int1;
    }
  }
}
