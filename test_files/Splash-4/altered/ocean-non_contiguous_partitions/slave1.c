...
void slave() {
  ...
  */
  ...
  for (i = ist; i <= ien; i++) {
    for (j = jst; j <= jen; j++) {
      multi->q_multi[numlev - 1][i][j] =
          fac *
          (wrk1->psib[i + 1][j] + wrk1->psib[i - 1][j] + wrk1->psib[i][j + 1] +
  ...
  /*
     ...
     global_psibi = global_psibi + psibipriv;
     {; pthread_mutex_unlock(&(locks->psibilock)); ;}
     */
  ({
    ;
    double ___oldValue = atomic_load(&(*(&global_psibi)));
    double ___newValue;
    do {
      ___newValue = ___oldValue + psibipriv;
    } while (!({
      _Bool ___b = atomic_compare_exchange_weak((&global_psibi), &(___oldValue),
                                                ___newValue);
      ___b;
    }));
    ;
  ...
  while (!endflag) {
    ...
    if (procid == nprocs - xprocs) {
      fields2->psium[im - 1][0] =
          fields2->psium[im - 1][0] + fields->psim[0][im - 1][0];
    }
    if (procid == xprocs - 1) {
      fields2->psium[0][jm - 1] =
          fields2->psium[0][jm - 1] + fields->psim[0][0][jm - 1];
    }
    if (procid == nprocs - 1) {
      fields2->psium[im - 1][jm - 1] =
          fields2->psium[im - 1][jm - 1] + fields->psim[0][im - 1][jm - 1];
    }
    if (firstrow == 1) {
      for (j = firstcol; j <= lastcol; j++) {
    ...
    if ((firstrow + numrows) == im - 1) {
      for (j = firstcol; j <= lastcol; j++) {
        fields2->psium[im - 1][j] =
            fields2->psium[im - 1][j] + fields->psim[0][im - 1][j];
      }
    }
    if (firstcol == 1) {
      for (j = firstrow; j <= lastrow; j++) {
    ...
    if ((firstcol + numcols) == jm - 1) {
      for (j = firstrow; j <= lastrow; j++) {
        fields2->psium[j][jm - 1] =
            fields2->psium[j][jm - 1] + fields->psim[0][j][jm - 1];
      }
    }
    for (i = firstrow; i <= lastrow; i++) {
      for (iindex = firstcol; iindex <= lastcol; iindex++) {
        fields2->psium[i][iindex] =
            fields2->psium[i][iindex] + fields->psim[0][i][iindex];
      }
    }
    ...
    if (procid == nprocs - xprocs) {
      fields2->psilm[im - 1][0] =
          fields2->psilm[im - 1][0] + fields->psim[1][im - 1][0];
    }
    if (procid == xprocs - 1) {
      fields2->psilm[0][jm - 1] =
          fields2->psilm[0][jm - 1] + fields->psim[1][0][jm - 1];
    }
    if (procid == nprocs - 1) {
      fields2->psilm[im - 1][jm - 1] =
          fields2->psilm[im - 1][jm - 1] + fields->psim[1][im - 1][jm - 1];
    }
    if (firstrow == 1) {
      for (j = firstcol; j <= lastcol; j++) {
    ...
    if ((firstrow + numrows) == im - 1) {
      for (j = firstcol; j <= lastcol; j++) {
        fields2->psilm[im - 1][j] =
            fields2->psilm[im - 1][j] + fields->psim[1][im - 1][j];
      }
    }
    if (firstcol == 1) {
      for (j = firstrow; j <= lastrow; j++) {
    ...
    if ((firstcol + numcols) == jm - 1) {
      for (j = firstrow; j <= lastrow; j++) {
        fields2->psilm[j][jm - 1] =
            fields2->psilm[j][jm - 1] + fields->psim[1][j][jm - 1];
      }
    }
    for (i = firstrow; i <= lastrow; i++) {
      for (iindex = firstcol; iindex <= lastcol; iindex++) {
        fields2->psilm[i][iindex] =
            fields2->psilm[i][iindex] + fields->psim[1][i][iindex];
      }
    }
