...
void maketree(long ProcessId) {
  ...
  if (ProcessId == 0) {
    Local[ProcessId].mycelltab[Local[ProcessId].myncell++] = Global_G_root;
  }
  Local[ProcessId].Current_Root = (nodeptr)Global_G_root;
  for (pp = Local[ProcessId].mybodytab;
       pp < Local[ProcessId].mybodytab + Local[ProcessId].mynbody; pp++) {
    p = *pp;
    if (Mass(p) != 0.0) {
      Local[ProcessId].Current_Root = (nodeptr)loadtree(
          p, (cellptr)Local[ProcessId].Current_Root, ProcessId);
    } else {
      {
        ;
        pthread_mutex_lock(&(Global_io_lock));
        ;
      };
...
void printtree(nodeptr n) {
  ...
  if (Type(n) == CELL) {
    ...
    for (k = 0; k < l->num_bodies; k++) {
      p = Bodyp(l)[k];
      printf("Body #%2ld: Num = %2ld, Level = %ld, ", p - bodytab, k, Level(p));
      PRTV("Pos", Pos(p));
      printf("\n");
    }
  } else {
...
bool intcoord(long xp[NDIM], vector rp) {
  ...
  inb = TRUE;
  for (k = 0; k < NDIM; k++) {
    xsc = (rp[k] - Global_rmin[k]) / Global_rsize;
    if (0.0 <= xsc && xsc < 1.0) {
      xp[k] = floor(IMAX * xsc);
    } else {
      inb = FALSE;
