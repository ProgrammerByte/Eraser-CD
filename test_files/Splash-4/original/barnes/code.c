...
int main(int argc, string argv[]) {
  ...
  printf("RESTTIME      = %12lu\t%5.2f\n",
         Global->tracktime - Global->partitiontime - Global->treebuildtime -
             Global->forcecalctime,
...
void ANLinit() {
  ...
  Global = (struct GlobalMemory *)({
    void *mem = malloc(sizeof(struct GlobalMemory));
    assert(mem);
    mem;
  });
  ;
...
void SlaveStart() {
  ...
  */
  ProcessId = ({
    ;
    uint64_t ___x = atomic_fetch_add(&(Global->current_id), 1);
    ;
    ___x;
  });
  ;
