...
int main(int argc, char *argv[]) {
  ...
  while ((c = getopt(argc, argv, "B:C:p:D:sth")) != -1) {
    if (c == 'B') {
      ...
      printf("Usage: CHOLESKY <options> file\n\n");
      printf("options:\n");
  ...
  gp = (struct gpid *)({
    void *mem = malloc(sizeof(struct gpid));
    assert(mem);
    mem;
  });
  ;
  ...
  { pthread_mutex_init(&(Global_waitLock), NULL); }
  { pthread_mutex_init(&(Global_memLock), NULL); }
  MallocInit(P);
  ...
  CreateBlockedMatrix2(M, num_alloc, T, firstchild, child, PERM, INVP, domain,
                       partition);
  ...
  printf("    0    %10.0ld\n", Global_runtime[0]);
  if (do_stats) {
    maxt = avgt = mint = Global_runtime[0];
    for (i = 1; i < P; i++) {
      if (Global_runtime[i] > maxt) {
        maxt = Global_runtime[i];
      }
      if (Global_runtime[i] < mint) {
        mint = Global_runtime[i];
      }
      avgt += Global_runtime[i];
    }
    avgt = avgt / P;
    for (i = 1; i < P; i++) {
      printf("  %3ld    %10ld\n", i, Global_runtime[i]);
...
void Go() {
  ...
  /*
    {; pthread_mutex_lock(&(Global_waitLock)); ;}
      MyNum = gp->pid;
      gp->pid++;
    {; pthread_mutex_unlock(&(Global_waitLock)); ;}
  */

  MyNum = ({
    ;
    uint64_t ___x = atomic_fetch_add(&(gp->pid), 1);
    ;
    ___x;
  });
  ...
  lc = (struct LocalCopies *)({
    void *mem = malloc(sizeof(struct LocalCopies) + 2 * PAGE_SIZE);
    assert(mem);
    mem;
  });
  lc->freeUpdate = NULL;
  ...
  if ((MyNum == 0) || (do_stats)) {
    Global_runtime[MyNum] = lc->runtime;
  }
  if (MyNum == 0) {
    gp->initdone = lc->rs;
    gp->finish = lc->rf;
  ...
  for (p = P - 1; p >= 0; p--)
    for (d = LB.proc_domains[p]; d < LB.proc_domains[p + 1]; d++) {
      ...
      MigrateMem(&LB.row[LB.col[first]],
                 (LB.col[LB.domains[d] + 1] - LB.col[first]) * sizeof(long), p);
