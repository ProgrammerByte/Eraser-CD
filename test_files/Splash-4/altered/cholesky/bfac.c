  ...
  for (js = 0; js < n; js += BS) {
    ...
    for (is = jl; is < n; is += BS) {
      ...
      for (ks = jl; ks < is; ks += BS) {
        ...
        OneMatmat(lc->blktmp, &A[ks + n * js], &A[is + n * ks], kl - ks,
                  jl - js, il - is, n, n);
      }
    ...
    for (js = 0; js < n1; js += BS) {
      ...
      for (is = 0; is < n3; is += BS) {
        ...
        for (ks = jl; ks < n1; ks += BS) {
          ...
          if (kl > n1)
            kl = n1;
          OneMatmat(lc->blktmp, &A[ks + js * n1], &B[is + ks * n3], kl - ks,
                    jl - js, il - is, n3, n1);
        }
      }
    }
  ...
  if (n2 * n3 <= BS * BS) {
    ...
    for (is = 0; is < n3; is += BS) {
      ...
      for (ks = 0; ks < n2; ks += BS) {
        ...
        OneMatmat(lc->blktmp, &A[ks * n1], &C[is], n1, kl - ks, il - is, n3,
                  n1);
      }
    }
  }
...
void CopyBlock(double *B, double *dest, long n3, long is, long ks, long il,
               long kl /*, long MyNum, struct LocalCopies *lc*/) {
  long i, k, bs;
  ...
  if (n1 * n2 <= BS * BS) {
    ...
    for (is = 0; is < n1; is += BS) {
      ...
      for (ks = 0; ks < n2; ks += BS) {
        ...
        OneMatmat(lc->blktmp, &A[ks * n1], &C[is], is, kl - ks, il - is, n1,
                  n1);
...
void OneLower(double *A, double *C, long n1, long n2, long n3) {
  ...
  for (j = 0; j < n1 - 1; j += 2) {
    for (k = 0; k < n2 - 3; k += 4) {
      ...
      last = dest0 + n1 - j;
      *dest0++ -=
          a_j0k0 * *b0++ + a_j0k1 * *b1++ + a_j0k2 * *b2++ + a_j0k3 * *b3++;
      dest1 = dest0 + n3;
      while (dest0 != last) {
        t0 = *dest0;
        t1 = *dest1;
...
void FindBlockUpdate(long domain, long bli, long blj, double **update,
                     long *stride) {
  long i;
  ...
  *update =
      &domain_update[into_j * update_len - into_j * (into_j + 1) / 2 + into_i];
