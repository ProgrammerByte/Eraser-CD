...
void CreateBlockedMatrix2(SMatrix M, long block_ub, long *T, long *firstchild,
  ...
  LB.proc_domain_storage =
      (double **)MyMalloc(LB.n_domains * sizeof(double *), DISTRIBUTED);
  for (i = 0; i < LB.n_domains; i++)
    LB.proc_domain_storage[i] = NULL;

  /* one dummy column for each domain */
  LB.partition_size =
      (long *)MyMalloc((LB.n + LB.n_domains + 1) * sizeof(long), DISTRIBUTED);
  LB.col =
      (long *)MyMalloc((LB.n + LB.n_domains + 1) * sizeof(long), DISTRIBUTED);

  LB.entry = (Entry *)({
    void *mem = malloc(LB.entries_allocated * sizeof(Entry));
    assert(mem);
    mem;
  });
  ;
  ...
  LB.row = (long *)({
    void *mem = malloc(LB.entries_allocated * sizeof(long));
    assert(mem);
    mem;
  });
  ;
  ...
  LB.renumbering =
      (long *)MyMalloc((LB.n + LB.n_domains) * sizeof(long), DISTRIBUTED);
  ...
  printf("Supers: ");
  DumpSizes(LB, domain, node);
  printf("Blocks: ");
  DumpSizes(LB, domain, LB.partition_size);
  ...
  for (super = 0; super < LB.n; super += node[super])
    if (domain[super]) {
      FindSuperStructure(M, super, PERM, INVP, firstchild, child, structure, nz,
                         &n_nz);
      FindDomStructure(super, nz, n_nz);
    } else {
      for (j = super; j < super + node[super]; j += LB.partition_size[j]) {
        FindBlStructure(M, j, PERM, INVP, firstchild, child, structure, nz);
...
void FindDummyDomainStructure(long which_domain) {
  ...
  while (row < LB.col[col + 1]) {
    ...
    while (LB.row[row] >= current_block && LB.row[row] < current_block_last &&
           row < LB.col[col + 1])
      row++;
  }
  ...
  for (j = 0; j < LB.n; j += LB.partition_size[j])
    if (!LB.domain[j]) {
      for (b = LB.col[j]; b < LB.col[j + 1]; b++) {
        ...
        BLOCK(b)->nz =
            (double *)MyMalloc(size * sizeof(double), BLOCK(b)->owner);
        for (i = 0; i < size; i++)
          BLOCK(b)->nz[i] = 0.0;
      }
    }
      ...
      for (b = LB.col[col]; b < LB.col[col + 1]; b++) {
        for (i = 0; i < BLOCK(b)->length; i++) {
          if (BLOCK(b)->structure)
            ...
            row = LB.row[b] + i;
          BLOCK(b)->nz[i + j1 * BLOCK(b)->length] = scatter[row];
          scatter[row] = 0.0;
        }
      }
    }
