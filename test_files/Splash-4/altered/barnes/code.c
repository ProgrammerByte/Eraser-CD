...
int main(int argc, string argv[]) {
  ...
  ;
  {
    ...
    for (i = 0; i < (NPROC)-1; i++) {
      ...
      Error = pthread_create(&__tid__[__threads__++], NULL, SlaveStart, NULL);
      if (Error != 0) {
  ...
  printf("TREEBUILDTIME = %12lu\t%5.2f\n", Global_treebuildtime,
         ((float)Global_treebuildtime) / Global_tracktime);
  printf("FORCECALCTIME = %12lu\t%5.2f\n", Global_forcecalctime,
         ((float)Global_forcecalctime) / Global_tracktime);
  printf("RESTTIME      = %12lu\t%5.2f\n",
         Global_tracktime - Global_partitiontime - Global_treebuildtime -
             Global_forcecalctime,
...
void tab_init() {
  ...
  for (i = 0; i < NPROC; ++i) {
    Local[i].ctab = (cellptr)({
      void *mem = malloc((maxcell / NPROC) * sizeof(cell));
      assert(mem);
      mem;
    });
    ;
    Local[i].ltab = (leafptr)({
      void *mem = malloc((maxleaf / NPROC) * sizeof(leaf));
      assert(mem);
      mem;
    });
    ;
  ...
  Local[0].mybodytab = (bodyptr *)({
    void *mem = malloc(NPROC * maxmybody * sizeof(bodyptr));
    assert(mem);
    mem;
  });
  ;
  ...
  Local[0].mycelltab = (cellptr *)({
    void *mem = malloc(NPROC * maxmycell * sizeof(cellptr));
    assert(mem);
    mem;
  });
  ;
  Local[0].myleaftab = (leafptr *)({
    void *mem = malloc(NPROC * maxmyleaf * sizeof(leafptr));
    assert(mem);
    mem;
  });
  ;

  CellLock = (struct CellLockType *)({
    void *mem = malloc(sizeof(struct CellLockType));
    assert(mem);
    mem;
  });
  ;
  {
    int i;
    for (i = 0; i < (MAXLOCK); i++)
      pthread_mutex_init(&((CellLock->CL)[i]), NULL);
  };
}
...
void SlaveStart() {
  ...
  /*   {; pthread_mutex_lock(&(Global_CountLock)); ;};
       ProcessId = Global_current_id++;
     {; pthread_mutex_unlock(&(Global_CountLock)); ;};
  */
  ProcessId = ({
    ;
    uint64_t ___x = atomic_fetch_add(&(Global_current_id), 1);
    ;
    ___x;
  });
  ;
...
void testdata() {
  ...
  Local[0].tnow = 0.0;
  bodytab = (bodyptr)({
    void *mem = malloc(nbody * sizeof(body));
    assert(mem);
    mem;
  });
  ;
...
void stepsystem(long ProcessId) {
  ...
  Cavg = (real)Cost(Global_G_root) / (real)NPROC;
  Local[ProcessId].workMin = (long)(Cavg * ProcessId);
  Local[ProcessId].workMax =
      (long)(Cavg * (ProcessId + 1) + (ProcessId == (NPROC - 1)));
  ...
  find_my_bodies((nodeptr)Global_G_root, 0, BRC_FUC, ProcessId);
  ...
  for (i = 0; i < NDIM; i++) {
    if (Global_min[i] > Local[ProcessId].min[i]) {
      Global_min[i] = Local[ProcessId].min[i];
    }
    if (Global_max[i] < Local[ProcessId].max[i]) {
      Global_max[i] = Local[ProcessId].max[i];
    }
  }
  {
    ;
  ...
  if (ProcessId == 0) {
    ...
    for (i = 0; i < NDIM; i++) {
      if (Global_rsize < Global_max[i]) {
        Global_rsize = Global_max[i];
      }
    }
    ADDVS(Global_rmin, Global_min, -Global_rsize / 100000.0);
...
void find_my_initial_bodies(bodyptr btab, long nbody, long ProcessId) {
  ...
  if (ProcessId >= extra) {
    offset = (Local[ProcessId].mynbody + 1) * extra +
             (ProcessId - extra) * Local[ProcessId].mynbody;
  }
    ...
    for (i = 0; i < l->num_bodies; i++) {
      if (work >= Local[ProcessId].workMin - .1) {
        if ((Local[ProcessId].mynbody + 2) > maxmybody) {
          error("find_my_bodies: Processor %ld needs more than %ld bodies; "
                "increase fleaves\n",
                ProcessId, maxmybody);
        }
        Local[ProcessId].mybodytab[Local[ProcessId].mynbody++] = Bodyp(l)[i];
      }
      work += Cost(Bodyp(l)[i]);
...
void setbound() {
  ...
  for (p = bodytab; p < bodytab + nbody; p++) {
    for (i = 0; i < NDIM; i++) {
      if (Pos(p)[i] < Local[0].min[i])
        Local[0].min[i] = Pos(p)[i];
      if (Pos(p)[i] > Local[0].max[i])
        Local[0].max[i] = Pos(p)[i];
    }
  }

  SUBV(Local[0].max, Local[0].max, Local[0].min);
  for (i = 0; i < NDIM; i++)
    if (side < Local[0].max[i])
      side = Local[0].max[i];
  ADDVS(Global_rmin, Local[0].min, -side / 100000.0);
  Global_rsize = 1.00002 * side;
  SETVS(Global_max, -1E99);
  SETVS(Global_min, 1E99);
}

void Help() {
  printf("There are a total of twelve parameters, and all of them have default "
         "values.\n");
  printf("\n");
  printf("1) infile (char*) : The name of an input file that contains particle "
         "data.  \n");
  ...
  printf("\n");
  printf("    Each of these numbers can be separated by any amount of "
         "whitespace.\n");
  printf("\n");
  printf("2) nbody (int) : If no input file is specified (the first line is "
         "blank), this\n");
  printf("    number specifies the number of particles to generate under a "
         "plummer model.\n");
  ...
  printf("\n");
  printf("4) outfile (char*) : The name of the file that snapshots will be "
         "printed to. \n");
