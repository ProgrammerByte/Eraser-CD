...
void PreAllocateFO(long MyNum, struct LocalCopies *lc) {
  ...
  AllBlocks[MyNum] = (struct BlockList **)MyMalloc(
      LB.n_partitions * sizeof(struct BlockList *), MyNum);
  DiagBlock[MyNum] = (struct BlockList **)MyMalloc(
      LB.n_partitions * sizeof(struct BlockList *), MyNum);
  ...
  for (i = LB.proc_domains[MyNum]; i < LB.proc_domains[MyNum + 1]; i++) {
    ...
    if (update_size) {
      LB.proc_domain_storage[i] =
          (double *)MyMalloc(update_size * sizeof(double), MyNum);
...
void PerformUpdate(struct BlockList *above_bl, struct BlockList *below_bl,
  ...
  if (is_diag) {
    ...
    BLMod(below_bl->length, LB.partition_size[below_bl->col], below_bl->nz,
          destination, lc);

    if (destination == lc->updatetmp) {

      ScatterUpdateFO(below_bl->length, below_bl->structure, below_bl->length,
                      below_bl->structure, BLOCK(dest_block)->length,
    ...
    if (below_bl->length == BLOCK(dest_block)->length)
      ...
      FindRelativeIndices(below_bl->structure, below_bl->length,
                          BLOCK(dest_block)->structure, lc->relative);
    ...
    BMod(above_bl->length, LB.partition_size[above_bl->col], below_bl->length,
         above_bl->nz, below_bl->nz, destination, lc);

    if (destination == lc->updatetmp) {

      ScatterUpdateFO(below_bl->length, relative_i, above_bl->length,
                      relative_j, BLOCK(dest_block)->length, lc->updatetmp,
                      BLOCK(dest_block)->nz);
    }
  }
...
void DistributeUpdateFO(long which_domain, long MyNum, struct LocalCopies *lc) {
  ...
  for (bi = LB.col[LB.n + which_domain]; bi < LB.col[LB.n + which_domain + 1];
       bi++) {
...
void HandleUpdate2FO(long which_domain, long bli, long blj, long MyNum,
  ...
  ScatterUpdateFO2(BLOCK(bli)->length, relative_i, BLOCK(blj)->length,
                   relative_j, stride, BLOCK(dest_block)->length, update,
                   BLOCK(dest_block)->nz);
...
void BlockReadyFO(long block, long MyNum, struct LocalCopies *lc) {
  ...
  if (BLOCKROW(block) == BLOCKCOL(block)) {
    ...
    if (DiagBlock[MyNum][column]) {
      ...
      BDiv(diagbl->length, BLOCK(block)->length, diagbl->nz, BLOCK(block)->nz,
           lc);
      BlockDoneFO(block, MyNum, lc);
    }
  }
...
void BlockDoneFO(long block, long MyNum, struct LocalCopies *lc) {
  ...
  if (scatter_decomposition) {
    ...
    /* send to row */
    for (i = 0; i < P_dimj; i++)
      Send(block, block, 0, 0, (struct Update *)NULL, P_row + i * P_dimi, MyNum,
           lc);

    /* send to column */
    for (i = 0; i < P_dimi; i++)
      if (i != P_row)
        Send(block, block, 0, 0, (struct Update *)NULL, i + P_col * P_dimi,
             MyNum, lc);
  } else {
    for (i = 0; i < P; i++)
