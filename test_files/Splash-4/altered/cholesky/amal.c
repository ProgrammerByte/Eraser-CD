...
void Amalgamate2(long join, SMatrix M, long *T, long *nz, long *node,
  ...
  free(tree_firstchild);
  free(tree_sibling);
  free(tree_original_firstchild);
  free(tree_original_sibling);
  free(next_in_super);
  free(member_of);
  free(super_parent);
  ...
  printf("%.0f/%.0f (%.2f) ops before/after amalgamation\n", g_ops_before,
         work_tree[M.n], work_tree[M.n] / (double)g_ops_before);
...
void ConsiderMerge(long join, long super, SMatrix M, long *nz, long *node,
  ...
  while (1) {
    ...
    PMOD(node[super], nz[super] - node[super], nz[super] - node[super], &dummy,
         &dummy, &time_before);
    PADD(nz[super] - node[super], nz[super] - node[super], &dummy,
         &time_before);
    ...
    time_after = 0;
    PDIV(node[super] + node[parent], node[super] + nz[parent], &dummy, &dummy,
         &time_after);
