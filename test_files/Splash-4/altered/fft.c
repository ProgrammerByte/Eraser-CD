...
_Atomic long Global_id;
pthread_mutex_t Global_idlock;

long *Global_transtimes;
long *Global_totaltimes;
unsigned long Global_starttime;
...
double TouchArray(double *x, double *scratch, double *u, double *upriv,
                  long MyFirst, long MyLast);
double CheckSum(double *x);
void InitX(double *x);
void InitU(long N, double *u);
void InitU2(long N, double *u, long n1);
long BitReverse(long M, long k);
void FFT1D(long direction, long M, long N, double *x, double *scratch,
           double *upriv, double *umain2, long MyNum, long *l_transtime,
           long MyFirst, long MyLast, long pad_length, long test_result,
           long dostats);
void TwiddleOneCol(long direction, long n1, long j, double *u, double *x,
                   long pad_length);
void Scale(long n1, long N, double *x);
void Transpose(long n1, double *src, double *dest, long MyNum, long MyFirst,
               long MyLast, long pad_length);
...
int main(int argc, char *argv[]) {
  ...
  while ((c = getopt(argc, argv, "p:m:n:l:stoh")) != -1) {
    if (c == 'p') {
      ...
      printf("  -t  : Perform FFT and inverse FFT.  Test output by comparing "
             "the\n");
      printf("        integral of the original data to the integral of the "
             "data that\n");
  ...
  x = (double *)({
    void *mem =
        malloc(2 * (N + rootN * pad_length) * sizeof(double) + PAGE_SIZE);
    assert(mem);
    mem;
  });
  ;
  ...
  ;
  umain = (double *)({
    void *mem = malloc(2 * rootN * sizeof(double));
    assert(mem);
    mem;
  });
  ;
  ...
  Global_transtimes = (long *)({
    void *mem = malloc(P * sizeof(long));
    assert(mem);
    mem;
  });
  ;
  Global_totaltimes = (long *)({
    void *mem = malloc(P * sizeof(long));
    assert(mem);
    mem;
  });
  ...
  if (dostats) {
    ...
    for (i = 1; i < P; i++) {
      ...
      printf("  %3ld        %10ld     %10ld      %8.5f\n", i,
             Global_totaltimes[i], Global_transtimes[i],
  ...
  printf("Total time with initialization    : %16lu\n",
         Global_finishtime - Global_starttime);
  printf("Total time without initialization : %16lu\n",
         Global_finishtime - Global_initdonetime);
  printf("Overall transpose time            : %16ld\n", transtime);
  printf("Overall transpose fraction        : %16.5f\n",
         ((double)transtime) / (Global_finishtime - Global_initdonetime));
  printf("\n");
...
void SlaveStart() {
  ...
  {
    ;
    pthread_mutex_lock(&(Global_idlock));
    ;
  };
  MyNum = Global_id;
  Global_id++;
  ...
  MyNum = ({
    ;
    uint64_t ___x = atomic_fetch_add(&(Global_id), 1);
    ;
    ___x;
  });
  {
    ;
    pthread_mutex_unlock(&(Global_idlock));
    ;
  };
  ...
  FFT1D(1, M, N, x, trans, upriv, umain2, MyNum, &l_transtime, MyFirst, MyLast,
        pad_length, test_result, dostats);
...
void FFT1D(long direction, long M, long N, double *x, double *scratch,
  ...
  for (j = MyFirst; j < MyLast; j++) {
    ...
    TwiddleOneCol(direction, n1, j, umain2, &scratch[2 * j * (n1 + pad_length)],
                  pad_length);
  }
